...
class RimePreviewPanel(QWidget):
    ...
    def _show_candidates(self, candidates) -> None:
        ...
        for index, candidate_label in enumerate(self._candidate_cells):
            ...
            width = max(candidate_label.sizeHint().width(), weight_label.sizeHint().width())
            candidate_label.setFixedWidth(width)
            weight_label.setFixedWidth(width)
        ...
        self._details.setText(detail)
        if self._dictionary_index is not None and self._dictionary_index.state == "building":
